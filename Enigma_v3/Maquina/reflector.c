...
void leeCfgReflector (char archivo[], struct sReflector *pRef) /*abre el archivo de configuración del reflector y lee la cadena de caracteres que contiene el cableado de dicho reflector.*/
{
    FILE*ptr;
    char modo;
    ...
    ptr= fopen(archivo, "rt"); /*Abre el archivo de configuracion de reflector*/
     ...
     if (ptr==NULL)
    {
        MessageBox(0,"Error en el archivo reflector.cfg","FILE MISSING",MB_ICONERROR);
        exit(1);
    }
...
void 	dibujaReflector (struct sReflector reflector) /* dibuja en la interfaz de la versión 2 una representación esquemática del reflector de la máquina Enigma.*/
{
    int u,i;
    char aux[27]=" ";
