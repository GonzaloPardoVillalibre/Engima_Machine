...
void dibujaInterfazM3(void) /*Dibuja la interfaz de la maquina engima*/
{
    ...
    system("mode con cols=91 lines=46"); /*definicion del tamaño de la consola*/
...
void 	printLetraCifrada (char letra, FILE *ptCifrado, unsigned short num) /* Muestra la letra que recibe como parámetro en la ventana "Texto cifrado" de la interfaz. Escribe la letra que recibe como parámetro en el archivo de texto para el texto cifrado.*/
{
...
unsigned short 	printLetraPlana (char letra, FILE *ptPlano, unsigned short num) /* Muestra la letra que recibe como parámetro en la ventana "Texto plano" de la interfaz.Escribe la letra que recibe como parámetro en el archivo de texto para el texto plano.*/
{
    static int contador = 0;
    int size;
    ...
    if(num==3) /*Complicado algortimo que se encargar de imprimirme correctamente la letra en la version de tres rotores*/
    {
        ...
        if(contador==250)
        {
            MessageBox(0,"Ha sobrepasado el tamaño maximo del mensaje\n(250 caracteres)","INSTRUCCIONES",MB_ICONINFORMATION);
            exit(1);
        }
        if(contador%25==0 && contador!=0)
        {
