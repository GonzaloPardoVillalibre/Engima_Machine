...
void genRingstellung(int nrotor,unsigned short ring[]) /*Selecciona aleatoriamente la poscion de los anillos*/
{
    ...
    if(nrotor>3)
    {
        for (i=0; i<4; i++)
        {
            ring[i]= 1+(rand()%26); /*Genero un número del 1-26 que representaría una letra de entrada*/
...
void leeCfgRotor (struct sRotor *pRotor) /*Lee la configuracion de un rotor*/
{
    ...
    if (ptr==NULL)
    {
        MessageBox(0,"Error en el archivo cableadoRotores.cfg","FILE MISSING",MB_ICONERROR);
        exit(1);
    }
...
void configRotores (struct sListaRotores *lista, struct sConfig config) /*implementa la configuración de los tres rotores de la máquina Enigma*/
{
    int i,u;
    char aux[27]= {"0"};
...
void avanzaRotores (struct sListaRotores *pListaRotores)  /*implementa el avance que experimentan los rotores de la máquina Enigma.*/
{
...
unsigned short 	opRotor (unsigned short in, struct sRotor rotor, bool dir) /*mplanta la operación de cifrado que se realiza en un rotor.*/
{
    unsigned short i ;
